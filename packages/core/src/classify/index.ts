...
export const STANCE_SYSTEM_PROMPT = `You classify financial tweets to find a person's TRADE CALLS.

For each tweet, for EACH ticker ($CASHTAG) provided, output the AUTHOR'S OWN current directional stance on THAT ticker:
- "bull" = author is bullish / expects it up / is buying or holding for gains / defending it / mocking bears who were proven wrong.
- "bear" = author is bearish / expects it down / is selling or shorting / warning people to avoid it.
- "none" = the AUTHOR states no clear directional view: neutral news, an observation of how the market or others reacted, a comparison/benchmark, a question, or describing what OTHERS think.

CRITICAL: words like "short", "sell", "crash", "bear", "dump" do NOT by themselves mean the author is bearish. Judge the AUTHOR'S own view.

Comparisons: if a ticker is only named as a benchmark for another (e.g. "$A is the next $B"), the benchmark $B is "none".

Default to "none" when unsure. conviction is 0..1.
...
export async function classifyBatch(
  items: BatchItem[],
  { apiKey = requiredEnv('OPENAI_API_KEY'), model = optionalEnv('OPENAI_MODEL') ?? 'gpt-5.4-nano' } = {},
